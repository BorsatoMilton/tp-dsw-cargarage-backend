...
} from "../correo/correo.controller.js";
...
function sanitizeAlquilerInput(
  ...
  try {
    ...
    if (fechaInicio.getTime() < fechaAlquiler.getTime())
      throw new Error(
        "La fecha de inicio no puede ser menor a la fecha actual"
      );
    if (fechaDevolucion.getTime() < fechaInicio.getTime())
      throw new Error(
    ...
    res
      .status(400)
      .json({
        message: error instanceof Error ? error.message : "Error en fechas",
      });
  }
}
...
function calcularFechaLimite(
  ...
  return tiempoEntreFechas > 0
    ? new Date(fechaAlquiler.getTime() + tiempoEntreFechas * 0.7)
    : null;
}
...
async function findAll(req: Request, res: Response) {
  try {
    const alquileres = await em.find(
      Alquiler,
      {},
      { populate: ["locatario", "vehiculo", "vehiculo.propietario"] }
    );
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al obtener el Alquiler", error: error.message });
  }
}
...
async function findAllByVehicle(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error al obtener los alquileres por vehículo",
        error: error.message,
      });
  }
}
...
async function findAllByUser(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error al obtener los alquileres por usuario",
        error: error.message,
      });
  }
}
...
async function add(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al crear el alquiler", error: error.message });
  }
}
...
async function cancelRent(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al cancelar el alquiler", error: error.message });
  }
}
...
async function remove(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al eliminar el alquiler", error: error.message });
  }
}
