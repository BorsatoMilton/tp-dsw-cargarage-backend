...
function sanitizeVehiculoInput(
  ...
  req.body.sanitizedInput = {
    ...
    precioVenta:
      req.body.precioVenta !== "" ? Number(req.body.precioVenta) : null,
    precioAlquilerDiario:
      req.body.precioAlquilerDiario !== ""
        ? Number(req.body.precioAlquilerDiario)
        : null,
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    const vehiculos = await em.find(
      Vehiculo,
      { fechaBaja: null },
      { populate: ["modelo", "categoria", "marca", "propietario", "compra"] }
    );
...
async function findAllByUser(req: Request, res: Response) {
  try {
    ...
    const vehiculos = await em.find(
      Vehiculo,
      { propietario: propietario, fechaBaja: null },
      { populate: ["modelo", "categoria", "marca", "propietario", "compra"] }
    );
    res.status(200).json(vehiculos);
  } catch (error: any) {
...
async function findAllByCategory(req: Request, res: Response) {
  try {
    ...
    const vehiculos = await em.find(
      Vehiculo,
      { categoria: categoria, fechaBaja: null },
      { populate: ["modelo", "categoria", "marca", "propietario", "compra"] }
    );
    res.status(200).json(vehiculos);
  } catch (error: any) {
...
async function findOneById(id: string) {
  const vehiculo = await em.findOne(
    Vehiculo,
    { id: id },
    { populate: ["modelo", "categoria", "marca", "propietario"] }
  );
  try {
    return vehiculo;
  } catch (error: any) {
...
async function add(req: Request, res: Response) {
  try {
    const imagePaths = Array.isArray(req.files)
      ...
      : []; // a cada archivo que se envia se le va a poner el nombre, se trata de mapear, si no lo deja vacio. el imagepaths muestra los nombres del archivo si lo q se envia es un array
    if (
      !ObjectId.isValid(req.body.sanitizedInput.marca) ||
    ...
    const vehiculoData = {
      ...
      categoria: ObjectId.createFromHexString(
        req.body.sanitizedInput.categoria
      ),
      propietario: ObjectId.createFromHexString(
        req.body.sanitizedInput.propietario
      ),
      imagenes: imagePaths,
    };
